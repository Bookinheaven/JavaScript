...
module.exports = async (client, member, guild) => {
  ...
  if(data2.Welcome){
      ...
      let textChannel = member.guild.channels.cache.find(ch => ch.id === data2.Welcome)
    ...
    // let link = 'https://coverfiles.alphacoders.com/470/47086.png'
      ...
      textChannel.send(`${textdata}`,attachment).catch(err => {
          if(err == 'DiscordAPIError: Missing Access'){
              ...
              .setTitle('Missing Permissions! For Welcome Images!')
              ...
              member.guild.owner.user.send(embed).catch(err=>{
                console.log(err)
              })
          }
      })
