...
class Rank {
    ...
    async build(ops = { fontX: "Manrope", fontY: "Manrope" }) {
        ...
        if (discrim) {
            ...
            ctx.fillText(`#${discrim.substr(0, 4)}`, ctx.measureText(name).width + 20 + 315, 164);
        }
        ...
        if (this.data.level.display && !isNaN(this.data.level.data)) {
            ...
            ctx.font = `bold 48px ${ops.fontX}`;
            ctx.fillStyle = this.data.level.color;
            ctx.textAlign = "end";
            ctx.fillText(`#${Util.toAbbrev(parseInt(this.data.level.data))}`, 890, 82);
        }
        ...
        if (this.data.rank.display && !isNaN(this.data.rank.data)) {
            ...
            ctx.fillText(this.data.rank.displayText, 750 - ctx.measureText(Util.toAbbrev(parseInt(this.data.level.data)) || "-").width - 7 - ctx.measureText(this.data.level.displayText).width - 7 - ctx.measureText(Util.toAbbrev(parseInt(this.data.rank.data)) || "-").width, 82);

            ctx.font = `bold 48px ${ops.fontX}`;
            ctx.fillStyle = this.data.rank.color;
            ctx.textAlign = "end";              //- ctx.measureText(Util.toAbbrev(parseInt(this.data.level.data)) || "-").width - 7 - ctx.measureText(this.data.level.displayText).width
            ctx.fillText(`#${Util.toAbbrev(parseInt(this.data.rank.data))}`, 675 , 82);
        }
